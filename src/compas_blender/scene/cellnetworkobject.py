...
class CellNetworkObject(BlenderSceneObject, BaseCellNetworkObject):
    """A scene object for drawing volumetric mesh data structures in Blender.

    Parameters
    ----------
    cellnetwork : :class:`compas.datastructures.CellNetwork`
        The cellnetwork data structure.
    **kwargs : dict, optional
        Additional keyword arguments.

    """

    def __init__(self, cellnetwork, **kwargs: Any):
        super().__init__(cellnetwork=cellnetwork, **kwargs)
        self.vertexobjects = []
        self.edgeobjects = []
        self.faceobjects = []
        self.cellobjects = []
    ...
    def draw_vertices(
        ...
        for vertex in vertices or self.cellnetwork.vertices():  # type: ignore
            name = f"{self.cellnetwork.name}.vertex.{vertex}"  # type: ignore
            color = self.vertexcolor[vertex]  # type: ignore
            point = self.cellnetwork.vertices_attributes("xyz")[vertex]
    ...
    def draw_edges(
        ...
        for u, v in edges or self.cellnetwork.edges():  # type: ignore
            name = f"{self.cellnetwork.name}.edge.{u}-{v}"  # type: ignore
            color = self.edgecolor[u, v]  # type: ignore
            curve = conversions.line_to_blender_curve(
                Line(self.cellnetwork.vertices_attributes("xyz")[u], self.cellnetwork.vertices_attributes("xyz")[v])
            )
    ...
    def draw_faces(
        ...
        for face in faces or self.cellnetwork.faces():  # type: ignore
            name = f"{self.cellnetwork.name}.face.{face}"  # type: ignore
            color = self.facecolor[face]  # type: ignore
            points = [self.cellnetwork.vertices_attributes("xyz")[vertex] for vertex in self.cellnetwork.face_vertices(face)]  # type: ignore
    ...
    def draw_cells(
        ...
        for cell in cells or self.cellnetwork.cells():  # type: ignore
            name = f"{self.cellnetwork.name}.cell.{cell}"  # type: ignore
            ...
            vertices = [self.cellnetwork.vertices_attributes("xyz")[vertex] for vertex in vertices]
    ...
    def draw_vertexnormals(
        ...
        for vertex in vertices or self.cellnetwork.vertices():  # type: ignore
            name = f"{self.cellnetwork.name}.vertex.{vertex}.normal"  # type: ignore

            a = self.cellnetwork.vertices_attributes("xyz")[vertex]
            n = self.cellnetwork.vertex_normal(vertex)  # type: ignore
    ...
    def draw_facenormals(
        ...
        for face in faces or self.cellnetwork.faces():  # type: ignore
            name = f"{self.cellnetwork.name}.face.{face}.normal"  # type: ignore

            a = centroid_points([self.cellnetwork.vertices_attributes("xyz")[vertex] for vertex in self.cellnetwork.face_vertices(face)])  # type: ignore
            n = self.cellnetwork.face_normal(face)  # type: ignore
            b = add_vectors(a, scale_vector(n, scale))
    ...
    #         labels.append(
    #             {
    #                 "pos": self.cellnetwork.vertices_attributes("xyz")[vertex],
    #                 "name": f"{self.cellnetwork.name}.vertexlabel.{vertex}",
    ...
    #         labels.append(
    #             {
    #                 "pos": centroid_points([self.cellnetwork.vertices_attributes("xyz")[u], self.cellnetwork.vertices_attributes("xyz")[v]]),
    #                 "name": f"{self.cellnetwork.name}.edgelabel.{u}-{v}",
    ...
    #         labels.append(
    #             {
    #                 "pos": centroid_points([self.cellnetwork.vertices_attributes("xyz")[vertex] for vertex in self.cellnetwork.face_vertices(face)]),
    #                 "name": "{}.facelabel.{}".format(self.cellnetwork.name, face),
