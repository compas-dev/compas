...
def install(packages=None, clean=False):
    """Install COMPAS for Rhino8.

    Parameters
    ----------
    packages : list of str, optional
        List of packages to install or None to use default package list.
        Default is the result of ``installable_rhino8_packages``,
    ...
    .. code-block:: python
        ...
        import compas_rhino8.install
        compas_rhino8.install.install()
    ...
    """
    # version = compas_rhino8._check_rhino8_version(version)
    version = "8.0"

    # We install COMPAS packages in the site packages folder
    sitepackages_path = compas_rhino8._get_default_rhino8_sitepackages_path(version)
    ...
    for package in packages:
        ...
        package_path = compas_rhino8._get_package_path(importlib.import_module(package))
    ...
    print("\nInstalling COMPAS packages to Rhino8 {0} site-packages folder:".format(version))
    ...
    if exit_code == 0 and len(installed_packages):
        ...
        if not _run_post_execution_steps(after_rhino8_install(installed_packages)):
            exit_code = -1
...
@compas.plugins.plugin(category="install", pluggable_name="installable_rhino8_packages", tryfirst=True)
def default_installable_rhino8_packages():
    ...
    return compas_rhino8.INSTALLABLE_PACKAGES


@compas.plugins.pluggable(category="install", selector="collect_all")
def installable_rhino8_packages():
    """Provide a list of packages to make available inside Rhino8.

    Extensions providing Rhino8 features
    can implement this pluggable interface to automatically
    have their packages made available inside Rhino8 when
    ...
    -------
    :obj:`list` of :obj:`str`
        List of package names to make available inside Rhino8.
    """
    pass


@compas.plugins.pluggable(category="install", selector="collect_all")
def after_rhino8_install(installed_packages):
    """Allows extensions to execute actions after install to Rhino8 is done.

    Extensions providing Rhino8 features
    can implement this pluggable interface to perform
    additional steps after an installation to Rhino8 has
    been completed.
    ...
    >>> @compas.plugins.plugin(category='install')
    ... def after_rhino8_install(installed_packages):
    ...    # Do something after package is installed to Rhino8, e.g. install the COMPAS UI, etc
    ...    return [('compas_rhino8', 'COMPAS Rhino8 UI installed', True)]
...
def _update_bootstrapper(install_path, packages):
    ...
    compas_bootstrapper = compas_rhino8._get_bootstrapper_path(install_path)

    bootstrapper_data = compas_rhino8._get_bootstrapper_data(compas_bootstrapper)
...
def _filter_installable_packages(version, packages):
    if not packages:
        ...
        packages = sorted(set(itertools.chain.from_iterable(installable_rhino8_packages())))  # type: ignore
    return packages
...
if __name__ == "__main__":
    ...
    #     "-v",
    #     "--version",
    #     choices=compas_rhino8.SUPPORTED_VERSIONS,
    #     default=compas_rhino8.DEFAULT_VERSION,
    #     help="The version of Rhino8 to install the packages in.",
    # )
    ...
    compas_rhino8.INSTALLATION_ARGUMENTS = args

    install(packages=args.packages, clean=args.clean)
