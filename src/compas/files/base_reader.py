...
class BaseReader(object):
    ...
    def location(self):
        ...
        Parameters
        ----------
        self._adress : string or Pathlib object
            Adress specified either as an URL, string containing path or
            Pathlib object
        ...
        """
        if self.is_adress_url():
            pathobj = self._download(self._adress)
        else:
            if not isinstance(self._adress, Path):
                pathobj = Path(self._adress)
            else:
                pathobj = self._adress
    ...
    @property
    def is_binary(self):
        """ Tries to determine if a file is binary nor not using the
            binaryornot library
    ...
    def is_adress_url(self):
        ...
        """
        return str(self._adress).startswith('http')
    ...
    def iter_chunks(self, chunk_size=4096):
        """Yields lines from binary files
