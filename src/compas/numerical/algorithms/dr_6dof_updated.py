...
def dr_6dof_numba(network, dt=1.0, xi=1.0, tol=0.001, steps=100, geomstiff=0):
    ...
    edges, l, T0, l0, I, J, De, row, col, edgei, Kall, data, u_, v_, kiu_, kiv_, m = _create_edge_arrays(network, fdof, fdof_node, k_i)
    ...
    while ts <= steps and rnorm > tol:
        ...
        data, f = _wrapper(T0, u_, v_, IDXalpha, IDXbeta, IDXgamma, x_, Sbt, eye3, zero3, zero6, zero9, kiv_, kiu_,
                           Sr0, Sr1, Sr2, Sxu, Syu, Szu, Sxv, Syv, Szv, l0, Kall, I, J, De, data, count, geomstiff, f, X,
        ...
        x = _update(x, v, dt, IDXtra, IDXrot, Lambdaold, dLambda, fdof_node, fdof_axis, fdof_rot, fdof_rot_, ind)
...
def _create_edge_arrays(network, fdof, fdof_node, k_i):
    ...
    array
        Index of vertex u for each edge.
    array
        Index of vertex u for each edge.
    array
        Key to index array for vertex u of each edge.
    array
        Key to index array for vertex v of each edge.
    int
        Number of edges.
    ...
    I = zeros(12 * m, dtype=int64)
    J = zeros(12 * m, dtype=int64)
    De = zeros(m + 1, dtype=int64)
    de = 0
    col = []
    row = []
    edgei = []
    ...
    return edges, l, T0, l0, I, J, De, row, col, edgei, Kall, data, u_, v_, kiu_, kiv_, m
...
def _beam_triad(i, x_, IDXalpha, IDXbeta, IDXgamma, T0, Sbt):
    ...
    """ Construct the current set of beam end axes.
...
def _create_T(eye3, zero3, zero6, zero9, xe, ye, ze, Re, Sr0, Sr1, Sr2, Sxu, Syu, Szu, Sxv, Syv, Szv, theta, xu, yu, zu, xv, yv, zv, li):
    ...
    zero3 : array
        Zero array of length 3.
    zero6 : array
        Zero array of length 6.
    zero9 : array
        Zero array of length 9.
    xe : array
        Edge x-direction.
    ye : array
        Edge y-direction.
    ze : array
        Edge z-direction.
    Re : matrix
        Average nodal rotation matrix.
    Sr0 : matrix
        Skew symmetrix matrix from Re[:, 0].
    Sr1 : matrix
        Skew symmetrix matrix from Re[:, 1].
    Sr2 : matrix
        Skew symmetrix matrix from Re[:, 2].
    Sxu : matrix
        Skew symmetrix matrix from xu.
    Syu : matrix
        Skew symmetrix matrix from yu.
    Szu : matrix
        Skew symmetrix matrix from zu.
    Sxv : matrix
        Skew symmetrix matrix from xv.
    Syv : matrix
        Skew symmetrix matrix from yv.
    Szv : matrix
        Skew symmetrix matrix from zv.
    thete : array
        Thetas.
    xu : array
        Vertex u x-direction.
...
@jit(Tuple((f8[:, :], f8[:, :]))(f8, f8, f8[:], f8[:, :], f8[:, :], i8[:], i8[:], i8[:], i8, f8[:, :]),
     nogil=True, nopython=True)
def _element_forces(li, l0i, theta, T, f, I, J, De, i, K_):
    ...
    array
        Internal force array.
    array
        Local element internal force array.
    ...
    f[I[De[i]: De[i + 1]]] += fe[J[De[i]: De[i + 1]]]
...
def Kg2(r_i, o, B, l, x, r_0, Sr0, Sx, Sri, So, riT_B, riT_xe, B_ri, B_ri_xeT, xeT_Sri, B_Sri, Sri_xe, Sx_Sri):
    ...
    Sr0 : matrix
        Skew symmetrix matrix from Re[:, 0].
    Sx : matrix
        Skew symmetrix matrix from xe.
    Sri : matrix
        Skew symmetrix matrix from Re[:, i].
    So : matrix
        Skew symmetrix matrix from o.
    riT_B : matrix
        mdotm(rit, B).
    riT_xe : matrix
        mdotm(rit, xe).
    B_ri : matrix
        mdotm(B, ri).
    B_ri_xeT : matrix
        mdotm(mdotm(B, r2), mxe.transpose()).
    xeT_Sri : matrix
        mdotm(mxe.transpose(), Sri).
    Sri_xe : matrix
        mdotm(Sr1, xe).
    Sx_Sri : matrix
        mdotm(Sx, Sri).
...
def _geometric_stiffmatrix(zeros33, zero123, f_, B, L2, L3, theta, Sr0, Sr1, Sr2, Sxu, Syu, Szu, Sxv, Syv, Szv, Sx, Szuzv, Syvyu, T, Re, xe, Tu, Tv, li):
    ...
    ----------
    zeros33 : matrix
        Three by three zero matrix.
    zeros123 : matrix
        Twelve by three zero matrix.
    f_ : array
        Local element internal force array.
    B : matrix
        B matrix.
    ...
    Sx : matrix
        Skew symmetrix matrix from xe.
    Szuzv : matrix
        Skew symmetrix matrix from zu - zv.
    Syvyu : matrix
        Skew symmetrix matrix from yv - yu.
    T : matrix
        Trabsformation matrix T.
    Re : matrix
        Average nodal rotation matrix.
    xe : array
        Edge x-direction.
...
@jit(Tuple((f8[:], i8))(i8[:], i8[:], i8[:], f8[:], i8, f8[:, :], f8[:, :], i8, i8, f8[:, :], f8[:, :], f8[:, :], f8[:, :],
     f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:, :],
     f8[:, :], f8[:, :], f8[:, :], f8[:, :], f8[:], f8[:], f8[:, :], f8[:, :], f8[:, :], f8[:, :]),
     nogil=True, nopython=True)
def _data(I, J, De, data, count, l0, T, geomstiff, i, K_, zero33, zero123, Sr0, Sr1, Sr2, Sxu, Syu, Szu, Sxv, Syv, Szv,
          Sx, Szuzv, Syvyu, L2, L3, Tu, Tv, theta, xe, Re, B, l, f_):

    """ Calculate global stiffness matrix entries.
    ...
    De : array
        Indices for slicing I and J
    data : array
        Entries for sparse assembly of K.
    l0 : array
        Initial edge lengths.
    T : matrix
        Trabsformation matrix T.
    ...
    -------
    array
        Global stiffness matrix entries.
    int
        Index for data.
    ...
    for j in range(de):
        for k in range(de):
            data[count] = (Ke[J[De[i] + j]][J[De[i] + k]])
            count += 1

    return data, count


# this function still feels messy to me, could be reworked. - Better?
@jit(f8[:, :](f8[:, :], f8[:, :], f8, i8[:], i8[:], f8[:], f8[:], i8[:], i8[:], i8[:], i8[:], i8[:, :]), nogil=True, nopython=True)
def _update(x, v, dt, IDXtra, IDXrot, Lambdaold, dLambda, fdof_node, fdof_axis, fdof_rot, fdof_rot_, ind):
    ...
    dt : float
        Time step.
    IDXtra : array
        List of indices to access all translational DoF.
    IDXrot : array
        Array of indices to access all rotational DoF.
    Lambdaold : array
        Pre-made rotation vector of current configuration for given vertex.
    dLambda : array
        Pre-made rotation vector increment.
    fdof_node : array
        Indexing list with node numbers for all fdof.
    fdof_axis : array
        Indexing list with axis numbers for all fdof.
    fdof_rot : array
        All free rotational DoF.
    fdof_rot_ : array
        All indices of vertices with free rotational DoF.
    ind : matrix
        Indexing matrix.
    ...
    for i in range(len(fdof_rot_)):
        ...
        normqnew = norm_vector_numba(qnew)
        lambdanew = 2 * arctan(normqnew / q0new)
...
def _wrapper(T0, u_, v_, IDXalpha, IDXbeta, IDXgamma, x_, Sbt, eye3, zero3, zero6, zero9, kiv_, kiu_, Sr0, Sr1, Sr2,
             Sxu, Syu, Szu, Sxv, Syv, Szv, l0, Kall, I, J, De, data, count, geomstiff, f, X, l, m, zero33, zero123,
             Sx, Szuzv, Syvyu):

    """ Calculate stiffness and internal forces
    ...
    l0 : array
        Initial edge lengths.
    Kall : matrix
        3D-matrix of ocal element siffness matrices.
    I : array
        Indices for accessing f.
    J : array
        Indices for accessing f_.
    ...
    array
        Global stiffness matrix entries.
    array
        Global element internal force array.
    ...
    for i in range(m):
        ...
        f, f_ = _element_forces(l[i][0], l0[i][0], theta, T, f, I, J, De, i, K_)

        data, count = _data(I, J, De, data, count, l0, T, geomstiff, i, K_, zero33, zero123, Sr0, Sr1, Sr2, Sxu, Syu, Szu, Sxv, Syv, Szv, Sx, Szuzv, Syvyu, L2, L3, Tu, Tv, theta, xe, Re, B, l, f_)
...
if __name__ == "__main__":
    ...
    network.set_vertices_attributes(keys=[0], attr_dict=bcs)
    network.set_vertices_attributes([m], {'pz': 600})
    network.set_edges_attributes(attr_dict={'axisy': [0.0, 0.0, 1.0]})

    tic = time()
    X = dr_6dof_numba(network=network, dt=1.0, xi=1., tol=0.001, steps=100, geomstiff=0)
    print(X[-1, :])
    print(time() - tic)
"""
...
if __name__ == "__main__":
    ...
    network.set_vertices_attributes(attr_dict={'pz': 50})
    network.set_edges_attributes(attr_dict={'axisy': [0.0, 0.0, 1.0]})
