...
def sliver_points():
    ...
    R = Rz @ Rx
    points = pts_local @ R.T + np.array([100.0, -40.0, 7.0])
...
def test_pca_numpy_well_conditioned_matches_covariance_route():
    ...
    assert np.allclose(V @ C @ V.T, np.diag(eigenvalues), atol=1e-12)

    assert np.allclose(mean, points.mean(axis=0))


def test_pca_numpy_near_collinear_recovers_smallest_direction(sliver_points):
    points, normal = sliver_points
    ...
    angle = np.degrees(np.arccos(np.clip(abs(smallest @ normal), -1.0, 1.0)))
    assert angle < 1e-4
...
def test_bestfit_plane_numpy_near_collinear(sliver_points):
    points, normal = sliver_points
    ...
    angle = np.degrees(np.arccos(np.clip(abs(fitted @ normal), -1.0, 1.0)))
    assert angle < 1e-4
