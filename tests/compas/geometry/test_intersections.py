...
def test_intersection_sphere_line():
    ...
    assert TOL.is_allclose(ipt1, (11.634, 10.634, 0.500), tol=1e-3)
    assert TOL.is_allclose(ipt2, (-0.634, -1.634, 0.500), tol=1e-3)
...
def test_intersection_plane_circle():
    ...
    assert TOL.is_allclose(ipt1, (-6.165, 7.000, 0.000), tol=1e-3)
    assert TOL.is_allclose(ipt2, (12.165, 7.000, 0.000), tol=1e-3)
...
def test_intersection_circle_circle_xy():
    ...
    assert TOL.is_allclose(ipt1, (9.999, -0.142, 0.000), tol=1e-3)
    assert TOL.is_allclose(ipt2, (-6.999, 7.142, 0.000), tol=1e-3)
